...
export function AdminDashboard() {
    ...
    const filteredFindMatches = allMatch.filter((match) => {
        const teamNames = `${match.teamA?.name} ${match.teamB?.name}`;
        return teamNames.toLowerCase().includes(findMatch.toLowerCase());
    })
    ...
    async function addTeamToTournament(e) {
        e.preventDefault()
        if (!addTournamentName || !addTeamName) {
            setAddTeamError('Required field')
            return
        }
        try {
            const response = await axiosInstance.put('/tournament/addteam/' + addTournamentName, { teamId: addTeamName })
            setAddTeam(response.data)
            setAddTeamError('')
            setTournamentId("")
            setAddTournamentName("")
            setTeamId("")
            setAddTeamName("")
        } catch (error) {
    ...
    async function updateMatchResult(e) {
        e.preventDefault()
        if (!matchId || !winnerName || !result) {
            setResultError('Required field')
            return
        }
        try {
            const response = await axiosInstance.post('/match/updatematch/' + matchId, { winner: winnerName, result })
    ...
    return (
        <div className="max-w-2xl mx-auto p-6 space-y-8">
                        ...
                        <div className="border rounded bg-white shadow mt-1">
                            {filteredWinner.map((t) => (
                                <p
                                    key={t._id}
                                    className="px-2 py-1 hover:bg-gray-100 cursor-pointer"
                                    onClick={() => { setwinnerName(t._id); setWinner(t.name) }}
                                >
                                    {t.name}
                                </p>
                            ))}
            ...
            <form onSubmit={createPlayerStats} className="border rounded-lg p-4 space-y-3">
                    ...
                    {matchstatsId && !statsMatch && (
                        <div className="border rounded bg-white shadow mt-1">
                            {filteredMatches.map((m) => (
                                <p className="px-2 py-1 hover:bg-gray-100 cursor-pointer" key={m._id} onClick={() => { setMatchStatsId(m.teamA?.name + " vs " + m.teamB?.name); setStatsMatch(m._id) }}>{m.teamA?.name} vs {m.teamB?.name}</p>
                            ))}
                        </div>
                    )}
                </div>
                ...
                <div className='relative'>
                    <label className="block text-sm">Team</label>
                    <input type="text" placeholder='Search by name' value={team} onChange={(e) => { setTeam(e.target.value); setStatsTeam('') }} className="border rounded px-2 py-1 w-full" />
                    {team && !statsTeam && (
                        <div className='border rounded bg-white shadow mt-1'>
                            {filteredStatsTeam.map((t) => (
